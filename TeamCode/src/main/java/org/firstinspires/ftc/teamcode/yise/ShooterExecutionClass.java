    ...
    public class ShooterExecutionClass {
        ...
        // Last completed press (copy for safe retrieval by opmode)
        private final double[] lastCompletedPressShotTimes = new double[] { Double.NaN, Double.NaN, Double.NaN };
        private double lastCompletedPressAverage = Double.NaN;
        ...
        private synchronized void finalizePress() {
            ...
            for (int i = 0; i < 3; i++) {
                lastCompletedPressShotTimes[i] = currentPressShotTimes[i];
            }
        ...
        public synchronized double getGlobalAverageShot1() {
            return (countShot1 > 0) ? (sumShot1 / (double) countShot1) : Double.NaN;
        }

        public synchronized double getGlobalAverageShot2() {
            return (countShot2 > 0) ? (sumShot2 / (double) countShot2) : Double.NaN;
        }

        public synchronized double getGlobalAverageShot3() {
            return (countShot3 > 0) ? (sumShot3 / (double) countShot3) : Double.NaN;
        }

        // Overall average across all recorded shots
        public synchronized double getGlobalOverallAverage() {
            return (countAllShots > 0) ? (sumAllShotTimes / (double) countAllShots) : Double.NaN;
        }
